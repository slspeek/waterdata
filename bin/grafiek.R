...
fig <- fig %>% add_lines(x = ~Datum, y = ~Waterstand.gecorrigeerd, name = "grondwaterstand", yaxis = "y1", line = list(color = "rgb(144, 168, 120)"))
fig <- fig %>% add_bars(x = regendata$YYYYMMDD, y = regendata$RH, name = "dagneerslag", yaxis = "y2", marker = list(color = "rgb(39, 144, 176)"))
...
fig <- fig %>%
    layout(
        ...
        xaxis = list(
            rangeselector =
                list(buttons = list(
                    list(count = 1, label = "Dag", step = "day", stepmode = "backward"),
                    list(count = 7, label = "Week", step = "day", stepmode = "backward"),
                    list(count = 1, label = "Maand", step = "month", stepmode = "backward"),
                    list(count = 3, label = "3 M", step = "month", stepmode = "backward"),
                    list(count = 6, label = "6 M", step = "month", stepmode = "backward"),
                    list(count = 1, label = "Jaar", step = "year", stepmode = "backward"),
                    list(label = "Alles", step = "all")
...
htmltools::save_html(fig, file = paste(getwd(), "website", "index.html", sep = "/"))
